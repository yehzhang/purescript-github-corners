...
import CSS (Transformation(..), absolute, color, fromString, key, left, nil, offset, position, px, right, top, transform, transformOrigin)
...
corner u bc oc p = a
    ...
    $ do
        svg
            ...
            $ do
                path ! d "M0,0 L115,115 L130,115 L142,142 L250,250 L250,0 Z" $ empty
                path
                ...
                path
                    ...
                    ! className "octo-body"
                    $ empty
        style $ text ".github-corner:hover .octo-arm{animation:octocat-wave 560ms ease-in-out}@keyframes octocat-wave{0%,100%{transform:rotate(0)}20%,60%{transform:rotate(-25deg)}40%,80%{transform:rotate(10deg)}}@media (max-width:500px){.github-corner:hover .octo-arm{animation:none}.github-corner .octo-arm{animation:octocat-wave 560ms ease-in-out}}"
  where
    styleAttribute = A.style <<< fromMaybe "" <<< renderedInline <<< render
    fill = key $ fromString "fill"
    border = key $ fromString "border"
